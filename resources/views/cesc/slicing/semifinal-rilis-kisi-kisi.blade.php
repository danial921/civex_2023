<div class="card mb-lg-3 mb-2 soal">
    <div class="card-body">
        <div class="mb-lg-2 mb-1 bd-highlight title">Rilis <span class="bold-blue">Kisi Kisi Semifinal</div>
        <div class="mb-lg-2 mb-2 bd-highligt sub-title">kisi kisi dapat di download pada link berikut :</div>
        <a href="submission-final">
            <button type="button" class="btn btn-daftar w-100">
                <img src="{{ asset('civex/img/icon/icon-paper.svg') }}" alt="your_image" width="20" height="20">
                <b>Kisi Kisi</b> Semifinal
            </button>
        </a>
    </div>
</div>
